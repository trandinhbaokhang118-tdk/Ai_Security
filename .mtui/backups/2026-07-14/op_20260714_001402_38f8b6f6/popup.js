...
async function init(){try{const state=await chrome.runtime.sendMessage({type:"GET_PROTECTION_STATE"});$("protection-enabled").checked=state?.enabled===true;if(!state?.enabled)return show("disabled");[currentTab]=await chrome.tabs.query({active:true,currentWindow:true});if(!currentTab||!validPage(currentTab.url))return show("unsupported");const entry=await chrome.runtime.sendMessage({type:"GET_TAB_RESULT",tabId:currentTab.id,url:currentTab.url});if(entry)render(entry,currentTab);else scan()}catch{$("error-message").textContent="Tiện ích gặp lỗi khi đọc trạng thái. Hãy tải lại extension.";show("offline")}}
$("protection-enabled").addEventListener("change",async(e)=>{await chrome.runtime.sendMessage({type:"SET_PROTECTION_STATE",enabled:e.target.checked});if(e.target.checked)init();else show("disabled")});$("rescan").addEventListener("click",()=>scan(true));$("retry-error").addEventListener("click",()=>scan(true));$("copy-url").addEventListener("click",async()=>{if(currentTab?.url){await navigator.clipboard.writeText(currentTab.url);$("copy-url").textContent="✓";setTimeout(()=>$("copy-url").textContent="⧉",1200)}});$("leave-page").addEventListener("click",()=>{if(currentTab?.id)chrome.tabs.update(currentTab.id,{url:"chrome://newtab"})});$("open-settings").addEventListener("click",()=>chrome.runtime.openOptionsPage());init();
