...
async def analyze_url(request: URLAnalysisRequest) -> URLAnalysisResponse:
    ...
    layers = [
        URLScoreLayer(layer="L1 · Nhận diện URL & giả mạo thương hiệu", score=core.layer_scores["lexical_identity"], status="completed", summary="Phân tích tên miền thật, thương hiệu, HTTPS, TLD, shortlink và homoglyph.", signals=sum(item.feature in {"brand_domain_mismatch", "deceptive_subdomain", "homoglyph", "brand_typosquatting", "punycode_domain", "ip_host", "risky_tld", "no_https", "is_shortlink"} for item in core.evidence), details=l1_details),
...
def _build_l1_details(signals, evidence_by_feature: dict[str, object]) -> list[dict]:
    parts = signals.parts
    return [
        ...
        _detail("Tuổi domain và danh tiếng nguồn ngoài", "Chưa truy vấn WHOIS/reputation feed", None, "Chưa chạy: phân tích nhanh ngoại tuyến không truy vấn WHOIS hoặc cơ sở dữ liệu website lừa đảo; cần tích hợp nguồn dữ liệu ngoài để có kết quả."),
    ]
