...
function send(message, timeoutMs = 6000) {
  return withTimeout(chrome.runtime.sendMessage(message), timeoutMs, "Tiến trình nền không phản hồi.");
}
function queryActiveTab() {
  return withTimeout(chrome.tabs.query({ active: true, currentWindow: true }), 2000, "Không đọc được tab hiện tại.");
}
...
async function waitForTabResult(token) {
  for (let count = 0; count < 30 && token === loadingToken; count += 1) {
    ...
    const entry = await chrome.runtime.sendMessage({ type: "GET_TAB_RESULT", tabId: currentTab.id, url: currentTab.url });
...
async function scan(force = false) {
  ...
  try {
    const entry = await chrome.runtime.sendMessage({ type: "ASSESS_URL", url: currentTab.url, tabId: currentTab.id, force });
...
async function init() {
  try {
    const state = await chrome.runtime.sendMessage({ type: "GET_PROTECTION_STATE" });
...
$("protection-enabled").addEventListener("change", async (e) => { await chrome.runtime.sendMessage({ type: "SET_PROTECTION_STATE", enabled: e.target.checked }); if (e.target.checked) init(); else { loadingToken += 1; show("disabled"); } });
$("rescan").addEventListener("click", () => scan(true));
$("retry-error").addEventListener("click", () => scan(true));
$("copy-url").addEventListener("click", async () => { if (currentTab?.url) { await navigator.clipboard.writeText(currentTab.url); $("copy-url").textContent = "✓"; setTimeout(() => $("copy-url").textContent = "⧉", 1200); } });
$("leave-page").addEventListener("click", () => { if (currentTab?.id) chrome.tabs.update(currentTab.id, { url: "chrome://newtab" }); });
