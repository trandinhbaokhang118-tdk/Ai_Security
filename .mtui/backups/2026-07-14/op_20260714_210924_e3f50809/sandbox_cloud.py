...
def make_sepay_qr_url(amount_vnd: int, reference: str) -> str:
    """Create a bank-transfer QR URL without exposing any SePay secret."""
    ...
    name = quote(settings.sepay_account_name, safe="")
    return f"{base}?acc={account}&bank={bank}&amount={amount_vnd}&des={content}&template=compact&accountName={name}"
...
def sepay_webhook(
    payload: SePayWebhook, db: DbSession = Depends(get_db),
    ...
    if order is None or payload.transferAmount < order.amount_vnd:
        return {"success": True, "ignored": "unknown_or_insufficient"}
