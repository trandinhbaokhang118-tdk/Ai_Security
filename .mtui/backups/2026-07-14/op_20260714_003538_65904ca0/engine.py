    ...
    def predict_url(self, url: str) -> PredictionResult:
        ...
        if self.url_session is not None:  # pragma: no cover - needs model
            try:
                ...
                version = "url_lgbm.onnx+url-rules-2"
            except Exception:
                score, version = rule_score, "heuristic-url-2"
        else:
            score, version = rule_score, "heuristic-url-2"
        return PredictionResult(_clip01(score), self._url_evidence(url, features), version)
    ...
    def predict_prompt(self, text: str) -> PredictionResult:
        ...
        if self.prompt_transformer_session is not None and self.prompt_tokenizer is not None:
            try:
                ...
                transformer_score = None
        prob = self._hybrid_score(transformer_score, lightweight_score, heuristic_prob)
        version = self._hybrid_version("prompt", transformer_score, lightweight_score)
        ev: list[Evidence] = []
        if prob >= 0.5:
            if transformer_score is not None and transformer_score >= 0.5:
            ...
            ev.append(Evidence(source="prompt_adapter", message="Không phát hiện prompt injection",
                               severity=Severity.INFO, contribution=0.02))
        return PredictionResult(_clip01(prob), ev, version)
