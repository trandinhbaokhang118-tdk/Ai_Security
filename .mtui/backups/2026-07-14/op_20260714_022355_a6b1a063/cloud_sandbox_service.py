...
class CloudSandboxService:
    ...
    def provision(self, session_id: str, user_id: str, expires_iso: str) -> tuple[str, str]:
        if not self.configured():
            raise RuntimeError("AWS sandbox chưa được cấu hình đầy đủ")
        params = {
            "ImageId": settings.aws_sandbox_ami_id,
            "InstanceType": settings.aws_sandbox_instance_type,
            "MinCount": 1,
            "MaxCount": 1,
