...
async function handleAssessUrl(url, tabId, force = false) {
  ...
  const promise = (async () => {
    try {
      ...
      const entry = { status: "complete", url, score: toDisplayScore(result.risk_score), result, requestId, startedAt, completedAt: Date.now(), latencyMs: Date.now() - startedAt };
  ...
  (async () => {
    try {
      switch (message.type) {
        ...
        case "GET_PROTECTION_STATE": return sendResponse(await settings());
