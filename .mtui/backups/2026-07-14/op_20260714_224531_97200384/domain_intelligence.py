...
class DomainIntelligenceService:
    ...
    def inspect(self, domain: str, url: str) -> DomainIntelligence:
        ...
        if not reasons and (rdap or certificates or reputation):
            reasons.append("Không phát hiện domain mới hoặc kết quả độc hại trong nguồn công khai đã truy vấn.")
        errors = [error for error in (rdap_error, certificate_error, reputation_error) if error]
        if errors and not (rdap or certificates or reputation):
