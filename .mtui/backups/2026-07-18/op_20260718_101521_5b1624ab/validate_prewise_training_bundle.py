...
def validate_row(
    ...
    if task in {"message-context-adapter", "web-context-adapter"}:
        ...
        if task == "web-context-adapter":
            if FORBIDDEN_HTML.search(user_blob):
        ...
        if RAW_URL.search(user_blob):
            raise ValueError(f"{path}:{line_number}: raw URL retained")
        if RAW_EMAIL.search(user_blob):
            raise ValueError(f"{path}:{line_number}: raw email address retained")
        if RAW_PHONE.search(user_blob):
            raise ValueError(f"{path}:{line_number}: raw phone number retained")
