...
_DATA_ID_PATTERN = re.compile(r"[A-Za-z0-9._~=-]{1,256}")
...
class ProviderRequestError(RuntimeError):
    """A safe, user-displayable external provider failure."""
    ...
    def inspect(self, data: bytes, filename: str) -> dict[str, Any]:
        ...
        try:
            ...
            if optional_size < minimum_optional_size:
                raise ValueError("Optional header ngắn hơn kích thước tối thiểu của định dạng PE")
            ...
            if directory_count > 4 and security_entry + 8 <= optional_offset + optional_size:
            ...
            score = 0
            parsed_section_count = min(section_count, _MAX_PE_SECTIONS)
            if section_count > _MAX_PE_SECTIONS:
                score += 20
                anomalies.append(
                    f"Số section khai báo vượt giới hạn phân tích an toàn: {section_count}."
                )
            headers_end = min(len(data), section_table_offset + (section_count * 40))
            for index in range(parsed_section_count):
                ...
                declared_raw_end = raw_pointer + raw_size
                raw_end = min(len(data), declared_raw_end)
                if raw_size > 0 and raw_pointer < headers_end:
                    score += 15
                    anomalies.append(f"Section {name} trỏ vào vùng header PE.")
                if raw_size > 0 and declared_raw_end > len(data):
                    score += 15
                    anomalies.append(f"Dữ liệu section {name} vượt quá kích thước tệp.")
                if raw_size > 0 and raw_pointer < len(data):
                    for previous_start, previous_end, previous_name in raw_ranges:
                        if raw_pointer < previous_end and raw_end > previous_start:
                            score += 18
                            anomalies.append(
                                f"Section {name} chồng lấn dữ liệu với section {previous_name}."
                            )
                            break
            ...
            if timestamp:
                try:
                    compiled = datetime.fromtimestamp(timestamp, tz=UTC)
...
class MetaDefenderProvider:
    ...
    def __init__(
        ...
        *,
        ...
        bounded_timeout = max(2.0, min(float(timeout_seconds), 120.0))
        self.timeout = httpx.Timeout(bounded_timeout)
    ...
    def _ensure_success(response: httpx.Response) -> None:
        ...
        if status == 401:
            message = "MetaDefender từ chối API key."
        elif status == 403:
            message = "MetaDefender không cho phép thao tác này với gói API hiện tại."
        elif status == 413:
            message = "Tệp vượt quá giới hạn dung lượng của MetaDefender."
        elif status == 429:
            reset = response.headers.get("x-ratelimit-reset-in") or response.headers.get(
                "retry-after"
            )
            suffix = f" Thử lại sau khoảng {reset} giây." if reset and reset.isdigit() else ""
            message = f"MetaDefender đã hết quota hoặc đang giới hạn tần suất.{suffix}"
        elif status >= 500:
            message = "MetaDefender đang tạm thời không khả dụng."
        else:
            message = f"MetaDefender trả về lỗi HTTP {status}."
        raise ProviderRequestError(message)

    def lookup_hash(self, sha256: str) -> dict[str, Any]:
        if not self.configured:
            return self.disabled_result()
        clean_hash = sha256.strip()
        if not _SHA256_PATTERN.fullmatch(clean_hash):
            raise ValueError("SHA-256 không hợp lệ")
        with httpx.Client(timeout=self.timeout, follow_redirects=False) as client:
            response = client.get(
                f"{self.base_url}/hash/{clean_hash.lower()}",
    ...
    def submit(self, data: bytes, filename: str) -> dict[str, Any]:
        if not self.configured:
            return self.disabled_result()
        raw_filename = PurePath(filename).name[:180] or "sample.exe"
        safe_filename = "".join(
    ...
    def report(self, data_id: str) -> dict[str, Any]:
        ...
        if not _DATA_ID_PATTERN.fullmatch(clean_id):
            raise ValueError("data_id không hợp lệ")
        encoded_id = quote(clean_id, safe="")
        with httpx.Client(timeout=self.timeout, follow_redirects=False) as client:
            response = client.get(
                f"{self.base_url}/file/{encoded_id}",
                headers=self._headers(),
            )
        self._ensure_success(response)
    ...
    def _normalize(
        ...
        *,
        ...
        queued_statuses = {"inqueue", "queued", "processing", "pending"}
        if raw_status in queued_statuses or 0 < progress < 100:
            status = "queued"
        elif progress >= 100:
            status = "completed"
        elif fallback_status == "known" and bool(scan):
            status = "known"
        else:
            status = fallback_status

        scan_details = scan.get("scan_details") or candidate.get("scan_details")
        ...
        if isinstance(scan_details, dict):
            for engine, detail in scan_details.items():
                if not isinstance(detail, dict):
                    continue
                threat = (
                    detail.get("threat_found")
                    or detail.get("scan_result_a")
                    or detail.get("verdict")
                    or detail.get("parent_verdict")
                    or detail.get("scan_result")
                )
                result_code = _safe_int(detail.get("scan_result_i"))
                threat_text = str(threat or "").strip()
                is_detection = result_code > 0 or threat_text.lower() not in _BENIGN_SCAN_LABELS
                if not is_detection:
                    continue
                detections.append(
                    {
                        "engine": str(engine)[:80],
                        "threat": (threat_text or "detected")[:160],
                    }
                )
