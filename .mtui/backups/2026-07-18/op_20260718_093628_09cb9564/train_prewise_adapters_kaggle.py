...
def main() -> None:
    ...
    # Tokenization needs the official tokenizer. Load the model together with the
    # tokenizer first, then render/filter datasets before constructing the trainer.
    from unsloth import FastLanguageModel

    log("Loading tokenizer/model for token audit...")
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name=args.model_name,
        max_seq_length=args.max_seq_length,
        load_in_4bit=False,
