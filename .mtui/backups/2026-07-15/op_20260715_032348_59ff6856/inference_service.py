...
class InferenceService:
    ...
    def assess_url(self, url: str) -> AssessResponse:
        ...
        try:
            domain = default_config().criteria and __import__("urllib.parse").parse.urlsplit(url if "://" in url else f"https://{url}").hostname
            if domain:
                add_domain_intelligence(observations, domain_intelligence_service.inspect(domain, url))
        except Exception as exc:
            for criterion_id in (1, 3, 4, 11, 12):
                observations.unavailable[criterion_id] = f"Domain intelligence unavailable: {type(exc).__name__}"
        v2_evidence = build_criteria_evidence(observations, default_config())
