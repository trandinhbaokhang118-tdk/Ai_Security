...
function show(section) {
    for (const id of ["disabled", "loading", "result", "offline"]) $(id).hidden = id !== section;
}
...
$("protection-enabled").addEventListener("change", async (event) => {
    const enabled = event.target.checked;
    await chrome.runtime.sendMessage({ type: "SET_PROTECTION_STATE", enabled });
    if (enabled) {
        show("loading");
        await init();
    } else {
