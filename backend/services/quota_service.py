        ...
        result = db.execute(
            text(
                ...
                INSERT INTO daily_quota_usage
                    (id, user_id, api_key_id, anonymous_id, usage_day, scan_count,
                     limit_snapshot, last_scan_at, created_at, updated_at)
                VALUES
                    (:id, :user_id, :api_key_id, :anonymous_id, :usage_day, 1,
                     :limit, :now, :now, :now)
    ...
    for _ in range(2):
        try:
            ...
            if user_id is not None:
                # A user's web, Extension, MCP and API traffic shares one plan
                # quota; api_key_id remains on the row for external attribution.
