"""Train the production lightweight phishing-text classifier from real CSV splits.

The Transformer path is useful when GPU budget is available. This script is the
CPU-safe serving path: it trains a TF-IDF + LogisticRegression model on the real
phishing text dataset and exports the exact ONNX artifact loaded by the gateway.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--train", default="data/phishing_text_train.csv")
    parser.add_argument("--validation", default="data/phishing_text_validation.csv")
    parser.add_argument("--test", default="data/phishing_text_test.csv")
...
def read_dataset(path: Path, *, max_chars: int) -> Dataset:
    ...
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            text = " ".join((row.get("text") or "").split())[:max_chars]
            if not text:
                continue
...
def main(argv: list[str] | None = None) -> None:
    ...
    train_data = read_dataset(train_path, max_chars=args.max_chars)
    validation_data = read_dataset(validation_path, max_chars=args.max_chars)
    test_data = read_dataset(test_path, max_chars=args.max_chars)
    pipe = train(
        train_data,
    ...
    output_path = out_dir / "mdeberta_text.onnx"
    ...
    metadata = {
        ...
        "purpose": "real-data CPU text phishing classifier for gateway serving",
    ...
    metadata_path = out_dir / "mdeberta_text.meta.json"
