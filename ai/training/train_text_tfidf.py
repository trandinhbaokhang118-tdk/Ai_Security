"""Train lightweight TF-IDF + Logistic Regression text classifier as alternative to transformer.

This is a lighter-weight alternative when GPU/disk space is limited.
For production quality, use train_text_transformer.py with mDeBERTa.

Usage:
    python -m ai.training.train_text_tfidf --data data/phishing_text_train.csv --out ai/models
"""
...
def train_model(X_train, y_train, max_features=10000, ngram_range=(1, 3)):
    ...
    pipeline = Pipeline([
        ("tfidf", TfidfVectorizer(
            ...
            stop_words=None,  # Keep all words for security detection
            min_df=2,
            max_df=0.9,
        )),
...
def save_metadata(output_path, metrics, train_files, train_size, val_size):
    ...
    metadata = {
        ...
        "architecture": "tfidf_logistic_regression",
        "purpose": "phishing text classification (lightweight alternative)",
        ...
        "note": "Lightweight TF-IDF model. For better accuracy, use mDeBERTa transformer.",
    }
...
def main():
    ...
    print(f"Training samples: {len(X_train)} (benign: {np.sum(y_train == 0)}, malicious: {np.sum(y_train == 1)})")
    print(f"Validation samples: {len(X_val)} (benign: {np.sum(y_val == 0)}, malicious: {np.sum(y_val == 1)})")
    ...
    output_path = output_dir / "text_tfidf_logreg.onnx"
