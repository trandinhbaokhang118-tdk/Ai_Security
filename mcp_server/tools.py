...
class MCPTools:
    ...
    def _assess_untrusted_content(
        self,
        content: str,
        modality: str,
        metadata: dict[str, Any],
    ):
        """Use the richer core hook when available, otherwise use its prompt gate."""
        assessor = getattr(self.service, "assess_untrusted_content", None)
        if callable(assessor):
            return assessor(content, modality, metadata)
        return self.service.assess_prompt(content)
    ...
    def assess_text(self, payload: TextInput) -> dict[str, Any]:
        ...
        if payload.content_type in {"webpage", "chat_message"}:
            result = self._assess_untrusted_content(
